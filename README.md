...
| . | Create an object that retreives its attributes (**objectIndex**, **objectType** (integer = 0, character = 1, sprite = 2), **objectTint**, **xCoordinateScale**, **yCoordinateScale**, **x**, **y**) from the current and following cells and updates itself when said cells are modified, or set the background to **objectIndex** (**objectType** == 3). **NOTE:** Sprites and audio files must be named "\<integer greater than -1\>.png" and backgrounds must be named "\<integer greater than 0\>.png". | objectIndex, objectType, objectTint, xCoordinateScale, yCoordinateScale, x, y |
